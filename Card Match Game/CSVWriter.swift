...
class CSVWriter {
    ...
    init(experimentName: String, participantID: Int) {
    ...
    // Write the header
    func writeHeader() {
        let header = "Exp Name,Group ID,Subject #,Block ID,Layout ID,Trial #,Goal Location,Goal Obj,Rep #,Touch Location,Touch Obj,Touch Time,Accuracy,\n"//block start/end time
...
//        #endif
    ...
    func writeHelper(str: String, app: Bool) {
        let name = "\(self.experimentName)-\(self.participantID).csv"
        let path = getDocumentsDirectory().appendingPathComponent(name)
        
        print("----------- APPENDING: " + String(app) + " ------------")
...
//
//            for item in items {
//                if item.hasPrefix("\(self.experimentName)-\(self.participantID)") {
//                    if item.contains("-") {
